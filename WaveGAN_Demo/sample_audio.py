...
def audio_sample_generator(filepath, window_length=16384, fs=16000):
    try:
        audio_data, sr = librosa.load(filepath, sr=fs)
        # Clip amplitude
    ...
    if audio_len < window_length:
        ...
        audio_len = len(audio_data)  # pad to window_length

    while True:
        if audio_len == window_length:
            # if only have a single frame's worth of audio, just yield the whole audio
            sample = audio_data
        else:
...
concept_audio_dataset_filepath = 'D:\\Projects\\Projects\\pytorch_Projects\\GAN\\Datasets\\concept cell dataset\\concept cell audio final\\'
get_all_audio_filepaths(concept_audio_dataset_filepath)
