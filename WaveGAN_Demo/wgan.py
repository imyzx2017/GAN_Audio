...
"""
    Generate sample_size number of output samples instead of batch size
"""
...
def calc_gradient_penalty(model_dis, real_data, fake_data, batch_size, lmbda, use_cuda=True):
    ...
    # alpha = alpha.extend(real_data.size())
...
def train_wgan(model_gen, model_dis, train_gen, valid_data, test_data,
    ...
    # Sample noise used for seeing the evolution of generated output sample throughout training
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx in range(batches_per_epoch):
            # Set model parameters to require gradients to be compute and stored
            for p in model_dis.parameters():
                p.requires_grad = True

            # Initialize the metric for this batch
            batch_history = {
                'discriminator':[],
                'generator':{}
            }

            # Discriminator Training Phase(Training D k times)
            for iter_d in range(discriminator_updates):
                ...
                if use_cuda:
                    noise = noise.cuda()
                # noise_v = autograd.no_grad(noise)
                ...
                D_cost_train, D_wass_train = compute_discr_loss_terms(
                    ...
                    latent_dim, lmbda, use_cuda, compute_grads=True)
                ...
                D_cost_valid, D_wass_valid = compute_discr_loss_terms(
                    ...
                    latent_dim, lmbda, use_cuda, compute_grads=False)
                ...
                D_cost_train_record = D_cost_train.cpu().detach()
                D_cost_valid_record = D_cost_valid.cpu().detach()
                D_wass_train_record = D_wass_train.cpu().detach()
                D_wass_valid_record = D_wass_valid.cpu().detach()

                batch_history['discriminator'].append({
                    'cost': D_cost_train_record.numpy(),
                    'wasserstein_cost': D_wass_train_record.numpy(),
                    'cost_validation': D_cost_valid_record.numpy(),
                    'wasserstein_cost_validation': D_wass_valid_record.numpy()
                })
                # batch_history['discriminator'].append({
            ...
            batch_history['generator']['cost'] = G_cost.cpu().detach().numpy() # [0]
        ...
        if (epoch + 1) % epochs_per_sample == 0:
            # Generate outputs for fixed latent sample
            LOGGER.info('Generating samples...')
            # sample_output = model_gen(sample_noise_v)
            sample_output = model_gen(sample_noise_v.cuda())
            if use_cuda:
                sample_output = sample_output.cuda()

            samples[epoch + 1] = sample_output.cpu().data.numpy()
            if output_dir:
